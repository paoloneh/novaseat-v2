...
1. Download `train_colab.ipynb` and `score_colab.ipynb` from this folder.
2. Go to [Google Colab](https://colab.research.google.com).
3. Click **File > Upload notebook** and select `train_colab.ipynb`.
4. Repeat for `score_colab.ipynb`.

That's it — both notebooks install their own dependencies automatically in the first cell.
...
2. Review the **Configuration** cell (cell 2) and adjust if needed:
...
4. When prompted, upload the IBM Telco Customer Churn CSV (`WA_Fn-UseC_-Telco-Customer-Churn.csv`).
   - Alternatively, comment out the upload block and set `CSV_PATH` to a Google Drive path.
5. The notebook will:
